...
int main(int argc, char* argv[])
{
  ...
  d_list_elem *ptr_sl, *tail;
  double_list **bucket, **temp;

  bucket = new double_list* [1];
  ...
  while(true){
    ...
    if(bucket[0] == NULL){// insert first msg
      bucket[0] = new double_list;
      bucket[0]->d_list_insert_front(new d_list_elem(msg, num, word));
      num_buc++;
    }
    else{
      for(index = 0 ; index < num_buc; index++){ // check msg number that already exists
	ptr_sl = bucket[index]->d_list_front();
	if(bucket[index]->d_list_get_data1(ptr_sl) == msg) break;
      }
      if(index == num_buc){// insert new msg
	if(num_buc == size_buc){ // if bucket is full
	  temp = new double_list*[2*size_buc];
	...
	bucket[index] = new double_list;
	bucket[index]->d_list_insert_front(new d_list_elem(msg, num, word));
	num_buc++;
      }
      else{// insert new packet of old msg
	ptr_sl = bucket[index]->d_list_front();
	tail = bucket[index]->d_list_tail();
	while(ptr_sl != tail){
	  if(bucket[index]->d_list_get_data2(ptr_sl) >=  num){
	    if(bucket[index]->d_list_get_data2(ptr_sl) == num) bucket[index]->d_list_replace(ptr_sl, new d_list_elem(msg, num, word));
	    else bucket[index]->d_list_insert_before(ptr_sl, new d_list_elem(msg, num, word));
	    break;
	  }
	  else{
	    ptr_sl = bucket[index]->d_list_next(ptr_sl);
	    continue;
	  }
	}
	if(ptr_sl == tail)
	  bucket[index]->d_list_insert_back(new d_list_elem(msg, num, word));
      }
    }
  }
  ...
  for(index = 0; index < num_buc ; index++){
    ptr_sl = bucket[index]->d_list_front();
    msg_c = bucket[index]->d_list_get_data1(ptr_sl);
    tail = bucket[index]->d_list_tail();
    fout << "- Message " << msg_c << endl;
    num_c = 1; // used for checking packet num
    while(ptr_sl != tail){
      if(bucket[index]->d_list_get_data2(ptr_sl) != num_c){
	fout << "WARNING: packet " << num_c << " of message " << msg_c << " is missing" << endl;
	num_c++;
	continue;
      }
      else{
	fout << bucket[index]->d_list_get_data3(ptr_sl) <<  endl;
	num_c++;
	ptr_sl = bucket[index]->d_list_next(ptr_sl);
	continue;
      }
