...
{
  return round (mulfloatint (load_avg, 100));
}
...
thread_get_recent_cpu (void) 
{
  ...
  return round (mulfloatint (recent_cpu, 100));
}

...
thread_calculate_priority (const struct thread *t)
{
  ...
  temp = round (subfloatint (addfloatint (-divfloatint (t->recent_cpu, 4), PRI_MAX),  2 * t->nice));
